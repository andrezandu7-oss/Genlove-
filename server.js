// ============================================
// SNS - SISTEMA NACIONAL DE SAÚDE (VERSÃO CORRIGIDA)
// ============================================
...
app.get('/dashboard', (req, res) => {
    ...
    '.modal-content input, .modal-content select { width: 100%; padding: 8px; margin: 10px 0; }' +
    '</style>' +
    '</head>' +
    '<body>' +
    '<div class="sidebar">' +
    ...
    '</div>' +
    '<div id="secaoCertificados" style="display: none;"><h1>Certificados</h1><p>Em breve...</p></div>' +
    '</div>' +
    ...
    'if (secao === "labs") { document.getElementById("secaoLabs").style.display = "block"; carregarLaboratorios(); }' +
    ...
    'function fecharModalLab() { document.getElementById("modalLab").style.display = "none"; }' +
    ...
    'const res = await fetch("/api/labs", { method: "POST", headers: { "Content-Type": "application/json", "Authorization": "Bearer " + token }, body: JSON.stringify(lab) });' +
    'const data = await res.json();' +
    'if (data.success) { alert("✅ Laboratório criado! API Key: " + data.lab.apiKey); fecharModalLab(); carregarLaboratorios(); }' +
    'else { alert("Erro: " + data.erro); }' +
    '}' +
    'async function carregarLaboratorios() {' +
    'const res = await fetch("/api/labs", { headers: { "Authorization": "Bearer " + token } });' +
    'const labs = await res.json();' +
    'document.getElementById("labsBody").innerHTML = labs.map(lab => "<tr><td>" + (lab.labId || "-") + "</td><td>" + lab.nome + "</td><td>" + lab.tipo + "</td><td>" + lab.provincia + "</td><td>" + (lab.ativo ? "✅ Ativo" : "❌ Inativo") + "</td><td><button onclick=\'desativarLab(\\"" + lab._id + "\\")\'>Desativar</button></td></tr>").join("");' +
    '}' +
    'async function desativarLab(id) { if (!confirm("Tem certeza?")) return; const res = await fetch("/api/labs/" + id, { method: "DELETE", headers: { "Authorization": "Bearer " + token } }); if (res.ok) { alert("Laboratório desativado"); carregarLaboratorios(); } }' +
    'async function carregarStats() { const res = await fetch("/api/stats", { headers: { "Authorization": "Bearer " + token } }); const data = await res.json(); document.getElementById("totalLabs").innerText = data.totalLabs || 0; document.getElementById("totalCerts").innerText = data.totalCertificados || 0; document.getElementById("certsHoje").innerText = data.certificadosHoje || 0; }' +
    'carregarStats();' +
