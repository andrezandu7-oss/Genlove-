...
// ============================================
// SISTEMA NACIONAL DE CERTIFICADOS MÉDICOS
...
app.use(cors({
    origin: ['http://localhost:3000', 'https://sns.gov.ao', 'https://lab.sns.gov.ao'],
    credentials: true
}));
...
// CONEXÃO MONGODB
// ============================================
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/saude-angola';
...
const labSchema = new mongoose.Schema({
    ...
    tipo: { 
        type: String, 
        enum: ['laboratorio', 'hospital', 'clinica', 'seguradora', 'ministerio', 'genlove'],
        required: true 
    },
    provincia: String,
    municipio: String,
    ...
    // Permissões
    permissoes: {
        tiposCertificado: [Number],  // [1,2,3,4,5]
        camposVisiveis: [String],     // ['prenom', 'nom', 'genotipo', etc.]
        formatoEspecial: String        // 'genlove' | 'completo' | 'restrito'
    },
...
const auditLogSchema = new mongoose.Schema({
    ...
    acao: { 
        type: String, 
        enum: ['EMISSAO', 'CONSULTA', 'VERIFICACAO', 'CRIACAO_LAB', 'ALTERACAO_LAB', 'LOGIN', 'LOGOUT']
    },
...
const statSchema = new mongoose.Schema({
    ...
    porProvincia: mongoose.Schema.Types.Mixed,
    porLaboratorio: mongoose.Schema.Types.Mixed,
    ativos: Number
});
...
function gerarApiKey() {
    return 'LAB-' + Date.now() + '-' + crypto.randomBytes(6).toString('hex').toUpperCase();
}

function gerarNumeroCertificado(tipo) {
    const ano = new Date().getFullYear();
    const random = crypto.randomBytes(3).toString('hex').toUpperCase();
    return `CERT-${tipo}-${ano}-${random}`;
}
...
app.get('/api/labs/:id', authMiddleware, async (req, res) => {
    try {
        const lab = await Lab.findById(req.params.id, { apiSecret: 0 });
        if (!lab) {
...
app.post('/api/certificados/emitir/:tipo', labAuthMiddleware, async (req, res) => {
    try {
        ...
        let validoAte = null;
        switch(tipo) {
            case 1: validoAte = null; break; // Vitalício
            case 2: validoAte = new Date(Date.now() + 180 * 24 * 60 * 60 * 1000); break;
            case 3: validoAte = new Date(dados.periodoFim); break;
            case 4: validoAte = new Date(Date.now() + 365 * 24 * 60 * 60 * 1000); break;
            case 5: validoAte = dados.dpp ? new Date(dados.dpp) : null; break;
        }
        ...
        // Gerar dados para Genlove
        const dadosGenlove = `${certificado.paciente.prenome}|${certificado.paciente.sobrenome}|${certificado.paciente.genero}|${certificado.dados.genotipo || ''}|${certificado.dados.grupoSanguineo || ''}`;
        certificado.dadosGenlove = dadosGenlove;

        // Cifrar dados completos
...
// ============================================
// SERVIÇO DE FRONTEND (Portal do Ministério)
// ============================================
...
async function createFirstAdmin() {
    try {
        ...
        if (!adminExists) {
            ...
            await admin.save();
            console.log('✅ Administrador criado: admin@saude.gov.ao / Admin@2025');
        }
    } catch (error) {
