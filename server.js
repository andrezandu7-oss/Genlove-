...
// CRIAÇÃO DOS MODELOS (AVEC FIX COLLECTION)
...
// 🔧 FIX COLLECTION - UNE FOIS SEULE
app.get('/api/fix-collection', authMiddleware, async (req, res) => {
    try {
        const collLab = mongoose.connection.db.collection('lab');
        const countRealLab = await collLab.countDocuments();
        console.log(`🔍 Collection 'lab': ${countRealLab} docs`);
        
        if (countRealLab > 0) {
            await collLab.rename('labs');
            res.json({ success: true, message: `✅ ${countRealLab} labs déplacés vers 'labs'` });
        } else {
            res.json({ success: true, message: 'ℹ️ Aucune correction nécessaire' });
        }
    } catch (e) {
