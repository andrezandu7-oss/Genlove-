...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Status badge (opcional, mas já existia) */
        .status-badge {
            background: #e8f5e9;
            color: #2e7d32;
...
<body>
    ...
    <div class="main">
        ...
        <div id="sec-dash" class="section active">
            <div class="header-flex">
                <h2>📊 Visão Geral</h2>
                <button class="btn-add" onclick="loadStats()">↻ Atualizar</button>
            </div>
        ...
        <!-- Seção Hospitais (placeholder) -->
        <div id="sec-hosp" class="section">
            <p>Gestão de hospitais (em breve)</p>
        </div>

        <!-- Seção Empresas (placeholder) -->
        <div id="sec-emp" class="section">
            <p>Gestão de empresas (em breve)</p>
        </div>
    </div>

    <!-- Modal de Novo Laboratório -->
    ...
    <script>
        ...
        // Carregar laboratórios (com botão PDF)
        async function loadLabs() {
            ...
            if (!labs.length) {
                tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:2rem;">Nenhum laboratório registado.</td></tr>';
                return;
            }
            tbody.innerHTML = labs.map(l => `
                <tr>
                    ...
                    <td>${l.nif || '—'}</td>
                    <td>${l.provincia || '—'}</td>
                    <td><span class="status-badge ${l.ativo ? '' : 'inativo'}">${l.ativo ? 'Ativo' : 'Inativo'}</span></td>
                    <td class="acoes">
                        <button class="btn-acao" onclick="gerarPdfLab(${JSON.stringify(l).replace(/"/g, '&quot;')})" title="Gerar PDF">📄</button>
                        <button class="btn-acao" onclick="alert('ID: ${l._id}')" title="Ver detalhes">👁️</button>
                    </td>
                </tr>
            `).join('');
        }

        // Função para gerar PDF do laboratório
        async function gerarPdfLab(labData) {
            try {
                ...
                a.href = url;
                a.download = `Laboratorio_${labData.nome.replace(/\s/g, '_')}.pdf`;
        ...
        // Salvar novo laboratório (já existente)
        async function salvarLaboratorio() {
            ...
            if (!dados.nome || !dados.nif || !dados.provincia) {
