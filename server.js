...
// ================================================
// DASHBOARD DO LABORATORIO (AVEC BOUTON IMPRIMER)
// ================================================
...
<body>
    ...
    <div class="main">
        ...
        <div id="secaoDashboard" class="secao ativa">
            ...
            <div id="graficoTipos" style="background:white; padding:20px; border-radius:10px;">
                <p>Carregando...</p>
            </div>
        </div>
        ...
        <div id="secaoCertificados" class="secao">
            ...
            <table>
                ...
                <tbody id="tabelaCertificados">
                    <tr>
                        <td colspan="5" style="text-align:center;">Carregando certificados...</td>
                    </tr>
                </tbody>
            </table>
        </div>
    ...
    <script>
        ...
        async function carregarDados() {
            try {
                // Carregar perfil do laboratório
        ...
        async function carregarStats() {
            try {
                const rStats = await fetch("/api/certificados/stats-detalhes", {
                    headers: { "x-api-key": key }
                });
                ...
                if (dStats.porTipo && dStats.porTipo.length > 0) {
                    let html = '<ul style="list-style:none; padding:0;">';
                    dStats.porTipo.forEach(item => {
                        html += `<li><strong>${tipos[item._id] || 'Desconhecido'}:</strong> ${item.count} certificados</li>`;
                    });
                    html += '</ul>';
                    document.getElementById("graficoTipos").innerHTML = html;
                }
        ...
        // =========================================
        // BLOQUEAR IMPRESSÃO DA PÁGINA (Ctrl+P)
        // =========================================
        window.addEventListener('keydown', function(e) {
            ...
            if ((e.ctrlKey || e.metaKey) && e.key === 'p') {
                ...
                alert('📌 Para imprimir um certificado, utilize o botão 🖨️ na lista de certificados.');
                return false;
            }
        });

        // Bloquear menu de contexto de impressão
        window.addEventListener('beforeprint', function(e) {
            e.preventDefault();
            alert('📌 Utilize o botão 🖨️ para imprimir certificados oficiais.');
            return false;
        });
        ...
        function mostrar(secao) {
            ...
            if (secao === "dashboard") {
                ...
                carregarStats();  // Atualizar stats ao voltar
            }
            if (secao === "certificados") {
                document.getElementById("secaoCertificados").classList.add("ativa");
                carregarCertificados();  // Atualizar lista ao voltar
            }
        }
