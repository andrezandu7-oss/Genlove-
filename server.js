...
app.get('/admin-dashboard', (req, res) => {
  ...
  <body>
    ...
    <script>
      ...
      // NOVA FUNÇÃO: gerar PDF do laboratório e abrir automaticamente
      ...
      // FUNÇÃO MODIFICADA: salvar laboratório e gerar PDF automaticamente
      async function salvarLaboratorio() {
        ...
        if (res.success) {
          // Preparar dados para o PDF (a rota espera responsavelTecnico)
          const pdfData = {
            nome: dados.nome,
            nif: dados.nif,
            tipo: dados.tipo,
      ...
      // Carregar stats ao iniciar (se a aba ativa for dash)
      window.onload = () => {
        if (document.getElementById('sec-dash').style.display !== 'none') loadStats();
      };
    </script>
  </body>
  </html>');
